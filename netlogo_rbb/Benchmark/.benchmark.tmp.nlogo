...
to setup
  ...
  let xs [ 5 10 10 5 10.5 ]
  let ys [ 5 5 2.5 10 13.5]
  let zoi_radii [ 5 5 2 2.5 1.5]

  ask patches [set nb-sharing 0]

  crt 5
  [
    setxy item who xs item who ys
    set rad item who zoi_radii
    set Ae pi * rad ^ 2
    set ZOI_factor 1
    set size rad * 2
    set color scale-color green size 0 20
  ]
  reset-ticks
end
...
to go
  ...
  to outputs
  clear-output
  output-print "setup, tree, x, y, zoi_radii, effective_area, ZOI_Factor"
  foreach sort turtles[ the-turtle ->
    ask the-turtle [
      output-print (sentence asymmetric-competition ", " who ", "  xcor  ", " ycor  ", " rad  ", " Ae  "," ZOI_Factor)
    ]
  ]
end
@#$#@#$#@
